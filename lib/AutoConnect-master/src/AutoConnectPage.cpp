...
String AutoConnect::_token_OPEN_SSID(PageArgument &args)
{
  ...
  if (creEntries > 0)
  {
    // ssidList = String(F("<h1>REdes</h1>"));
    ssidList = String(F("<h1>Redes guardadas.</b></h1><br/><a style=\"text-decoration: none;padding: 10px;font-weight: 600;font-size: 20px;background-color: #1883ba;border-radius: 6px;border: 2px solid #0016b0;color: #ffffff;\" href='/delwifi'>Olvidar red</a><br/>"));
    _scanCount = WiFi.scanNetworks(false, true);
  }
  else
    ssidList = String(F("<p><b>No hay redes guardadas.</b></p>"));
