...
#define AC_DBG_DUMB(...)               \
  do                                   \
  {                                    \
    AC_DEBUG_PORT.printf(__VA_ARGS__); \
  } while (0)
#define AC_DBG(...)                    \
  do                                   \
  {                                    \
    AC_DEBUG_PORT.print("[AC] ");      \
    AC_DEBUG_PORT.printf(__VA_ARGS__); \
  } while (0)
#else
#define AC_DBG(...)
...
#define AUTOCONNECT_MENU_TITLE "IMPULSE"
#endif // !AUTOCONNECT_MENU_TITLE

// URIs of AutoConnect menu collection
#define AUTOCONNECT_URI_CONFIG "/config"
#define AUTOCONNECT_URI_CONNECT "/connect"
#define AUTOCONNECT_URI_RESULT "/result"
#define AUTOCONNECT_URI_OPEN "/open"
#define AUTOCONNECT_URI_DISCON "/disc"
#define AUTOCONNECT_URI_RESET "/reset"
#define AUTOCONNECT_URI_SUCCESS "/success"
#define AUTOCONNECT_URI_FAIL "/fail"
#define AUTOCONNECT_URI_UPDATE "/update"
#define AUTOCONNECT_URI_UPDATE_ACT "/update_act"
#define AUTOCONNECT_URI_UPDATE_PROGRESS "/update_progress"
#define AUTOCONNECT_URI_UPDATE_RESULT "/update_result"
...
#define AC_HAS_FUNC(func)                        \
  ...
  private:                                       \
    ...
    typedef long two;                            \
    template <typename U>                        \
    static one test(decltype(&U::func));         \
    template <typename U>                        \
    static two test(...);                        \
                                                 \
  public:                                        \
